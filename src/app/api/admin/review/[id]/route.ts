...
import { updateReviewItem, deleteReviewItem } from "@/lib/repositories/review-repository";
...
async function resolveReviewId(context: RouteContext): Promise<string> {
...
function createErrorResponse(error: unknown): Response {
  if (error instanceof ZodError) {
    return Response.json(
      { ok: false, error: "INVALID_REVIEW_INPUT", issues: error.flatten() },
      { status: 400 },
    );
  }
  ...
  if (error instanceof Error) {
    ...
    if (error.message === "INVALID_REVIEW_ID") {
...
function revalidatePublicReviewPages(): void {
  revalidatePath("/");
...
export async function PATCH(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const reviewId = await resolveReviewId(context);
    ...
    const updated = await updateReviewItem(db, reviewId, input);
...
export async function DELETE(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const deleted = await deleteReviewItem(db, reviewId);
